    ...
    public static void sort(List<Stock> list) {
        list.sort(new Comparator<Stock>() {
            public int compare(Stock stock1, Stock stock2) {
                Comparator<Stock> stringComparator = Comparator.comparing(obj -> (String) obj.get("name"));
                Comparator<Stock> dateComparator = (d1, d2) ->
                        Long.compare(
                        getZeroTimeDate((Date) d2.get("date")).getTime(),
                        getZeroTimeDate((Date) d1.get("date")).getTime());
                Comparator<Stock> changeComparator = (c1, c2) -> getProfitPercentage(c2) - getProfitPercentage(c1);
                int result;
                return
                        (result = stringComparator.compare(stock1, stock2)) != 0 ? result :
                        (result = dateComparator.compare(stock1, stock2)) != 0 ? result :
                        changeComparator.compare(stock1, stock2);
            }
